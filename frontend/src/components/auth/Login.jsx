...
const API_BASE_URL = "https://backend-terassc.vercel.app";
  ...
  return (
    <div className="flex justify-center items-center w-full min-h-screen bg-[#F0F8FF] px-4 sm:px-6">
      <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl shadow-black/10 w-full max-w-[360px] sm:max-w-md text-center">
        ...
        <p className="text-gray-600 text-xs sm:text-sm mb-6">Student Center Reservation System</p>
        ...
        <form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          <div className="relative mb-4">
            <input
              ...
              required
            />
            <span onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-[#3f72af] cursor-pointer transition-colors">
              {showPassword ? <FaEyeSlash /> : <FaEye />}
            </span>
          </div>

          {/* Tombol Login */}
          <button type="submit" disabled={loading} className="bg-[#3f72af] text-white p-2 sm:p-3 rounded-lg text-sm font-semibold hover:bg-[#2b578c] transition-colors duration-300 disabled:opacity-50 w-full">
