...
import './DetailRuangan.css';

const DetailRuangan = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { bookings: allBookings, isLoaded, error } = useBookings();
  ...
  if (error) {
    return (
      <div className="w-full p-6 bg-white rounded-xl shadow-lg border border-gray-200 text-red-600 font-medium">
        Gagal memuat data peminjaman: {error}
      </div>
    );
  }
